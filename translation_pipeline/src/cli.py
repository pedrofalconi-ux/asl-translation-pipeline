...
def main():
    ...
    parser.add_argument(
        "-c",
        "--corpus",
        help='Path to the corpus CSV file. Will replace any ocurrences of "CORPUS_PATH" in the pipeline JSON.',
        default=os.path.join(here, "..", "data", "corpus", "corpus.csv"),
    )
    parser.add_argument(
        "-t",
        "--train-hash",
        help='Train artifact hash, if running a test pipeline. Will replace any ocurrences of "TRAIN_HASH" in the pipeline JSON.',
        default="",
    )
    parser.add_argument("-m", "--message", help="Execution message.", default=None)
    ...
    with open(args.pipeline, "r") as pipeline_json_file:
        ...
        pipeline_expression = pipeline_json.replace("CORPUS_PATH", args.corpus).replace(

...
class TransformersResultsElement(PipelineElement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self._checkpoint_path = os.path.realpath(os.path.join(self._train_link_folder, 'Checkpoints'))
        except KeyError:
    ...
    def process(self, data=None):
        ...
        with open(results_csv_path, 'w') as results_csv_file:
            ...
            csv_writer.writerow(
                ['PT', 'GI (Padrão Ouro)', 'GI (Gerado pela rede)', 'Score', 'Resultado']
            )
            ...
            bleu_output = self._bleu.compute(predictions=predictions, references=references, max_order=4)
            sacrebleu_output = self._sacrebleu.compute(predictions=predictions, references=references)
            meteor_output = self._meteor.compute(predictions=predictions, references=references)

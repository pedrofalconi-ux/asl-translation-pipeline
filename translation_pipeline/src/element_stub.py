...
class ElementStub():
    ...
    def process_and_pass_along(self, data=None, prev_cache_key=''):
        ...
        cache_key = None
        try:
            cache_key = hashlib.sha1((self.instance.get_cache_key() + prev_cache_key + str(self.instance.version)).encode('utf-8'))
        except AttributeError:
            # If element doesn't define a cache key function, use hash of
            # its parameter dictionary.
            cache_key = hashlib.sha1((str(self.params) + prev_cache_key + str(self.instance.version)).encode('utf-8'))
        ...
        if self.instance._force_cache_key:
            cache_key_used = self.instance._force_cache_key
            logger.warning(f'[{self.name}] Being forced to use cache key `{cache_key_used}`. Proceed with caution...')
        else:
            cache_key_used = cache_key

        output = cache.read_from_cache(cache_key_used) if not self.instance.dont_use_cache else None

        if output:
            logger.info(f'[{self.name}] Found and read cache key `{cache_key_used}`.')
        else:

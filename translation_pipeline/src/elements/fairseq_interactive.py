...
class InteractiveScoreElement(PipelineElement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._train_link_folder = os.path.join(
            get_artifact_directory(), self._train_hash
        )
        ...
        self._checkpoint_path = os.path.realpath(
            os.path.join(self._train_link_folder, "checkpoint_best.pt")
        )
    ...
    def _fairseq_interactive(self):
        ...
        file_out_name = os.path.join(
            get_artifact_directory(), file_basename_wout_ext + ".out"
        )
        ...
        if os.system(
            f'fairseq-interactive "{self._train_bin}" --path "{data_path}" {str_parameters} --remove-bpe < "{data_src}" > "{file_out_name}"'
        ):
    ...
    def _fairseq_score(self, file_name):
        ...
        # calcular o BLEU
        if os.system(
            f'fairseq-score --sys "{file_hypo}" --ref "{data_ref}" > "{file_bleu}"'
        ):

...
class FamososAugmentation(PipelineElement):
    ...
    def process(self, data):
        ...
        for (famoso_gr, famoso_gi) in list(zip(famosos_gr, famosos_gi)):
            ...
            combined_gr += "(\\b" + (''.join(lista_gr[-1:])) + "\\b))|"    #puts the last name in the expression for "gr" with an "or (|)" at the end so that the following names will also be caught
            match_famoso_gr[famoso_gr] += "(\\b" + (''.join(lista_gr[-1:])) + "\\b)"    #puts the last name in the dictionary value for "gr"

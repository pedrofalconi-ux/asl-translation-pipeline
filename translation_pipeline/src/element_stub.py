    ...
    def process_and_pass_along(self, data=None, prev_cache_key=""):
        ...
        try:
            cache_key = hashlib.sha1(
                (
                    self.instance.get_cache_key()
                    + prev_cache_key
                    + str(self.instance.version)
                ).encode("utf-8")
            )
            ...
            cache_key = hashlib.sha1(
                (str(self.params) + prev_cache_key + str(self.instance.version)).encode(
                    "utf-8"
                )
            )
        ...
        if self.instance._force_cache_key:
            ...
            logger.warning(
                f"[{self.name}] Being forced to use cache key `{cache_key_used}`. Proceed with caution..."
            )
        else:
            cache_key_used = cache_key

        output = (
            cache.read_from_cache(cache_key_used)
        ...
        if isinstance(output, dict):
            ...
            for key, val in output.items():
                logger.info(
                    f"[{self.name}] Pipeline bifurcation: sending data to `{key}`..."
                )
            ...
            if self.output["default"]:
                ...
                self.output["default"].process_and_pass_along(
                    output, prev_cache_key=cache_key
                )
            else:

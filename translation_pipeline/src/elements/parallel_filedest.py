...
class ParallelFileDestElement(PipelineElement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self._complete_gr_path = os.path.join(
                get_artifact_directory(), self._gr_path
            )
            os.makedirs(os.path.dirname(self._complete_gr_path), exist_ok=True)
            self._complete_gi_path = os.path.join(
                get_artifact_directory(), self._gi_path
            )
    ...
    def process(self, data):
        with open(self._complete_gr_path, "w") as self._gr_fd, open(
            self._complete_gi_path, "w"
        ) as self._gi_fd:
            for row in data:

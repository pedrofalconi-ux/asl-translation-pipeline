...
class PlacesAugmentation(PipelineElement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            new_places_total = (
                int(kwargs["total_places"]) if "total_places" in kwargs else 3
            )
    ...
    def process(self, data):
        ...
        list_places = (
            {}
        )  # dictionary with the lists for each type of place: Country, State and City
        countries = (
            []
        )  # list with every place that is a country (has the '&PAÍS' identifier) found in the 'lugares.csv' file
        states = (
            []
        )  # list with every place that is a states (has the '&ESTADO' identifier) found in the 'lugares.csv' file
        cities = (
            []
        )  # list with every place that is a city (has the '&CIDADE' identifier) found in the 'lugares.csv' file

        for (
            row
        ) in (
            self._reader
        ):  # goes through each place of the 'lugares.csv' file, checking what type of place it is by its identifier, and putting it in the correct list
        ...
        list_places[
            "PAÍS"
        ] = countries  # inserts the list of countries into the dictionary
        ...
        for (
            ...
            data
        ):  # Generate the augmentation for each sentence of the corpus, adding them to a set
            aug_phrases = self.generate(
                phrase, list_places, self._max_new_sentences, self._new_places_total
            )
        ...
        data = data + list(
            aug_data
        )  # concatenates the original data with the augmented data (turned into a list)
        return data
    ...
    def generate(self, data, places, max_new_sentences, max_num_places):
        ...
        splited_gr = self.split_phrase(
            gr, gr_places_found
        )  # splits the gr of the sentence into multiple sentences with one place in each
        splited_gi = self.split_phrase(
            gi, gi_pattern
        )  # splits the gi of the sentence into multiple sentences with one place in each
        ...
        random.shuffle(
            possible_places
        )  # shuffles the combinations in order to not always create the same structures
        possible_places = [k for k in possible_places if len(k) == len(set(k))][
            :max_new_sentences
        ]  # removes repeated cases (e.g ('BRASIL', 'BRASIL')) and only takes the specified amount of combinations

        if len(possible_places[0]) >= 1 and len(splited_gr) == len(
            splited_gi
        ):  # will only do the augmentation in case the element is not empty, i.e, there actually are places found in the sentence
            for (
                places_comb
            ) in (
                possible_places
            ):  # goes through every possible combination, which will be substitute the original places in the sentence
                ...
                for pattern_gr, pattern_gi, item, part_gr, part_gi in zip(
                    gr_places_found,
                    gi_places_found,
                    places_comb,
                    splited_gr,
                    ...
                    ex = re.search(
                        rf"([{valid_chars}_])+(?=&PAÍS|&ESTADO|&CIDADE)", item
                    ).group(
                        0
                    )  # searches for all the places found and puts the in a list (e.g "EU AMAR BRASIL&PAÍS" returns just "BRASIL")

                    new_phrase_gr.append(
                        re.sub(pattern_gr, ex, part_gr)
                    )  # makes the replacement of the nth place in the sentence in the nth partitioned sentence for the 'gr'
                    new_phrase_gi.append(
                        re.sub(pattern_gr + "&" + pattern_gi, item, part_gi)
                    )  # makes the replacement of the nth place in the sentence in the nth partitioned sentence for the 'gi'
                ...
                augmented_phrases.add(
                    ("".join(new_phrase_gr), "".join(new_phrase_gi))
                )  # concatenates the split augmented sentences into one sentence, like the original, but with the correct replacements
    ...
    def row_search(self, gr, gi, places):
        ...
        """
        pattern = rf"([{valid_chars}_]+&(PAÍS|ESTADO|CIDADE))"
        gr_places_found = []  # stores the cities/states/countries found in the sentence
        gi_places_found = (
            []
        )  # stores the type of places found in the sentence (city, state or country) according to the places found (same order)
        # e.g "SOU DE MANAUS NO AMAZONAS/SOU MANAUS&CIDADE EM AMAZONAS&ESTADO", would create the lists ('MANAUS', 'AMAZONAS')
        # and ('CIDADE', 'ESTADO')

        gi_pattern = (
            []
        )  # stores the patterns that will be used to replace the places in the 'gi'

        all_places = (
            places["PAÍS"] + places["ESTADO"] + places["CIDADE"]
        )  # concatenates all places in a single list

        for (
            place
        ) in (
            all_places
        ):  # for each place in list of places, checks if it's in the both the gr and gi of the sentence in equal amounts
    ...
    def split_phrase(self, phrase, patterns):
        ...
        for index, pattern in enumerate(patterns):
            ...
            try:  # if the split was made, then concatenates the first splitted sentence into the list
                ...
                ):  # concatenates the end of the sentence, which doesn't include one of the places to augment

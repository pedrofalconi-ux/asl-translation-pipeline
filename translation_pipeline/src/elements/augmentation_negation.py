...
class NegationAugmentation(PipelineElement):
    ...
    def process(self, data):
        ...
        for item in self._reader:
            list_negation[
                item[0]
            ] = ""  # insets every word in the list of negation in "list_negation"
        ...
        data_augmentation = list(
            data_augmentation
        )  # turns the set into a list to be used for the shuffel and concatenation
    ...
    def generate(self, list_negation, line):
        ...
        words_found_gr = (
            []
        )  # list where all the words that are both in the "gr" part of sentence and in the list of negated words will be put
        words_found_gi = (
            []
        )  # list where all the words that are both in the "gi" part of  sentence and in the list of negated words will be put

        words_gr = line[
            0
        ].split()  # splits the "gr" sentence into words in order to check if they are in the list
        words_gi = line[
            1
        ].split()  # splits the "gi" sentence into words in order to check if they are in the list
        previous_word = ""
        ...
        for item_gr in words_gr:
            ...
            if (
                previous_word == "NÃO"
            ):  # if preceded by "NÃO" then it adds "NÃO" + white space + word to list of words found
                if (
                    "NÃO_" + item_gr.upper() in list_negation
                    or "não_" + item_gr.lower() in list_negation
                ):
                    words_found_gr.append("NÃO " + item_gr.upper())
            else:
                if (
                    item_gr.upper() in list_negation or item_gr.lower() in list_negation
                ):  # if not preceded by "NÃO", then onlythe word itself will be added
        ...
        if (
            words_found_gr
            and words_found_gi
            and len(words_found_gr) == len(words_found_gi)
        ):
            number_of_words = len(
                words_found_gr
            )  # number of words that are in the list that were found in the sentence

            # if only one word was found then it returns the sentence negated
            if number_of_words == 1:

                if "NÃO_" == words_found_gi[0][:4] and (
                    "NÃO " == words_found_gr[0][:4] or "NÃO_" == words_found_gr[0][:4]
                ):  # if the word is negated then returns the sentence with the non-negated word
                    word_without_negation = words_found_gi[0][4:]
                    new_aug = (
                        re.sub(
                            r"\b" + words_found_gr[0] + r"\b",
                            word_without_negation,
                            line[0],
                        ),
                        re.sub(
                            r"\b" + words_found_gi[0] + r"\b",
                            word_without_negation,
                            line[1],
                        ),
                    )
                    lista.append(
                        new_aug
                    )  # inserts the augmented sentence into the list

                elif (
                    "NÃO_" != words_found_gi[0][:4]
                    and "NÃO " != words_found_gr[0][:4]
                    and "NÃO_" != words_found_gr[0][:4]
                ):  # if the word is non-negated then returns the sentence with the negated word
                    new_aug = (
                        re.sub(r"(\b" + words_found_gr[0] + r"\b)", r"NÃO \1", line[0]),
                        re.sub(r"(\b" + words_found_gi[0] + r"\b)", r"NÃO_\1", line[1]),
                    )
                    lista.append(
                        new_aug
                    )  # inserts the augmented sentence into the list
            ...
            # if more than one word was found then it returns all possible combinations of these words in the sentence (negated or not negated)
                ...
                list_of_words_caught_gr = (
                    []
                )  # this list will be used to avoid repeating the original sentence in the augmentation for "gr"
                list_of_words_caught_gi = (
                    []
                )  # this list will be used to avoid repeating the original sentence in the augmentation for "gi"
                ...
                re_words_gr = (
                    []
                )  # this list contains the regex expression for which word of the sentence that is in the list for "gr"
                re_words_gi = (
                    []
                )  # this list contains the regex expression for which word of the sentence that is in the list for "gi"
                ...
                for index, word_gi in enumerate(
                    set(words_found_gi)
                ):  # "words_found_gr/gi" is made into a set to eliminate repeated words

                    if (
                        word_gi[:4].upper() == "NÃO_"
                    ):  # in case the world is already negated then it'll add the negated word along a non-negated version of it in the "gi"
                        re_words_gi.append(
                            r"\b("
                            + word_gi.upper()
                            + "|"
                            + word_gi[4:].upper()
                            + r")\b"
                        )  # regex expression for both the negated and non-negated word
                        list_of_words_caught_gi.append(
                            [word_gi.upper()]
                        )  # inserts a list with the negated word as its first element
                        list_of_words_caught_gi[index].append(
                            word_gi[4:].upper()
                        )  # adds the non-negated word as its second element in the list for that word
                        ...
                        re_words_gi.append(
                            ...
                            + word_gi.upper()
                            + r")\b"
                        )  # regex expression for the non-negated word
                        list_of_words_caught_gi.append(
                            [word_gi.upper()]
                        )  # inserts a list with the original word as its first element
                        list_of_words_caught_gi[index].append(
                            "NÃO_" + word_gi.upper()
                        )  # adds the negated word as its second element in the list for that word

                # generates a regex expression just for the words found and puts the word itself and a negated version of it in "list_of_words_caught" (replacing what was in it before) for every word found
                for index, word_gr in enumerate(set(words_found_gr)):
                    if (
                        word_gr[:4].upper() == "NÃO_" or word_gr[:4].upper() == "NÃO "
                    ):  # if the words comes negated
                        re_words_gr.append(
                            r"\b("
                            + word_gr.upper()
                            + "|"
                        ...
                        list_of_words_caught_gr.append(
                            [word_gr.upper()]
                        )  # inserts a list with the negated word as its first element
                        list_of_words_caught_gr[index].append(
                            word_gr[4:].upper()
                        )  # adds the non-negated word as its second element in the list for that word
                ...
                next(
                    combinations_iterator_gr
                )  # this next is necessary in order to skip the first combination, i.e, the original sentence
                next(
                    combinations_iterator_gi
                )  # this next is necessary in order to skip the first combination, i.e, the original sentence

                combinations_gr = [
                    i for i in combinations_iterator_gr
                ]  # creates a list from the combinations
                random.shuffle(
                    combinations_gr
                )  # shuffles the list in order to not generate the sentences in a specific order
                combinations_gr = combinations_gr[
                    : self._max_new_sentences
                ]  # only generate the correct number of sentences from the number of combinations

                combinations_gi = [
                    i for i in combinations_iterator_gi
                ]  # creates a list from the combinations
                random.shuffle(
                    combinations_gi
                )  # shuffles the list in order to not generate the sentences in a specific order
                combinations_gi = combinations_gi[
                    : self._max_new_sentences
                ]  # only generate the correct number of sentences from the number of combinations
                ...
                for words_gr, words_gi in zip(combinations_gr, combinations_gi):
                    ...
                    for word_gr, word_gi in zip(words_gr, words_gi):
                        sentence_gr = re.sub(
                            re_words_gr[i], word_gr, sentence_gr
                        )  # makes the right replacement for which word in "gr"
                        sentence_gi = re.sub(
                            re_words_gi[i], word_gi, sentence_gi
                        )  # makes the right replacement for which word in "gi"
                        i += 1

                    lista.append(
                        (sentence_gr, sentence_gi)
                    )  # inserts the augmented sentence into the list

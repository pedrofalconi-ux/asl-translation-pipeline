...
class ApplyBpeElement(PipelineElement):
    ...
    def __init__(self, *args, **kwargs):
        ...
        -train_hash {string}: Train folder hash value (when using apply_bpe on the test pipeline).
        """
        ...
        if "train_hash" in kwargs:
            ...
            train_original_dir = os.path.join(
                os.path.dirname(get_artifact_directory()), train_hash
            )
            ...
            train_bpe_src = os.path.join(
                os.getcwd(), train_original_dir, "BPE", "bpe_code"
            )
            ...
            train_checkpoint_src = os.path.join(
                os.getcwd(), train_original_dir, "Checkpoints", "checkpoint_best.pt"
            )
            ...
            train_cfg_src = os.path.join(
                os.getcwd(), train_original_dir, "train_parameters.json"
            )
    ...
    def process(self, data=None):
        for set_corp in self._set_corp_list:
            for lang in [self._src, self._tgt]:
                file_to_apply_bpe_on = os.path.join(
                    self._preprocessed_path, set_corp + "." + lang
                )

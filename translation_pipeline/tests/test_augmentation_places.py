...
class TestPlacesAugmentation(unittest.TestCase):
    ...
    def test_data_without_augmentation(self):
        ...
        corpus = [
            (
                "COMO PODER PASSAR TEMPO [INTERROGAÇÃO]",
                "COMO PODER&POSSIBILIDADE PASSAR&TEMPO [INTERROGAÇÃO]",
            ),
            (
                "COMO PODER AGRADECER [INTERROGAÇÃO]",
                "COMO PODER&POSSIBILIDADE AGRADECER ELE [INTERROGAÇÃO]",
            ),
        ]
        augmentation = PlacesAugmentation(path=PATH_PLACES, max_new_sentences=50)
    ...
    def test_len_data_augmentation(self):
        ...
        corpus = [
            (
                "SAIR BRASIL PARA CHINA VOLTAR PARA BRASIL [PONTO]",
                "SAIR BRASIL&PAÍS PARA CHINA&PAÍS VOLTAR BRASIL&PAÍS [PONTO]",
            ),
            (
        ...
        length = 1000
        augmentation = PlacesAugmentation(path=PATH_PLACES, max_new_sentences=length)
        generated = augmentation.process(corpus)
